...
**Round-3/4 fixes (also in-branch):** gold answer key no longer leaks through `RETURNING *` mutation responses (shared `_strip_page` on submit/withdraw/manager actions); approval cap re-checked inside the loop (closes the 1→3 race that `FOR UPDATE` alone left open); **per-assignment-status edit-locking** now enforced — the original pair is locked once their assignment is `submitted` (incl. `needs_adjudication`), while the tier-3 adjudicator stays free; `send-back` re-opens submitted assignments for rework; `export_page` limited to annotator/manager/admin.

**Partial / still open:** **N4 connection-leak** (read handlers + `get_current_user` lack `try/finally`; a DB blip can exhaust the pool) — pending dedicated pass. `GOLD_CALIBRATION_PASS` is defined but the calibration gate isn't wired into assignment yet, and gold pages aren't excluded from adjudication (both part of #9, low impact).
