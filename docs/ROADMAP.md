...
**Partial / still open:** the per-box edit lock on `needs_adjudication` for the *original* tier-1/2 annotators is not yet enforced (left editable for the tier-3 adjudicator) — full per-assignment-status locking lands with #4. **N4 connection-leak** (read handlers + `get_current_user` lack `try/finally`; a DB blip can exhaust the pool) is a pending dedicated pass.
